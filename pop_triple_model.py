...
def conv_net_kelz(inputs, is_training, data_format='NCHW', batch_size=8, num_classes=88, scope=''):
    ...
    with slim.arg_scope(
        ...
        with slim.arg_scope([slim.batch_norm], is_training=is_training, data_format=data_format):
            ...
            print(net.shape)
            net = slim.max_pool2d(net, [3, 1], stride=[3, 1], scope=scope+'pool2', data_format=data_format)
            print(net.shape)
            ...
            print(net.shape)
            feature_map = slim.max_pool2d(net, [3, 1], stride=[3, 1], scope=scope+'pool3', data_format=data_format)
            ...
            print(net.shape)
            # was  64*1*51
            net = tf.reshape(net, (-1, 64*1*76), scope+'flatten4')
            print(net.shape)
