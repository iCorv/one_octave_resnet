...
val_dataset_tfrecord = glob.glob("./tfrecords-dataset/sigtia-configuration2-splits/fold_2/test/*.tfrecords")
...
def main(_):
    ...
    if train_and_val:
        train_spec = tf.estimator.TrainSpec(
            input_fn=lambda: dataset.tfrecord_triple_train_input_fn(train_dataset_tfrecord,
                                                             batch_size=hparams['batch_size'],
                                                             num_epochs=hparams['train_epochs']),
            max_steps=hparams['train_steps'])
        eval_spec = tf.estimator.EvalSpec(
            input_fn=lambda: dataset.tfrecord_triple_val_input_fn(val_dataset_tfrecord,
                                                           batch_size=hparams['batch_size'],
                                                           num_epochs=1),
    ...
    if eval_flag:
        eval_result = classifier.evaluate(
            input_fn=lambda: dataset.tfrecord_triple_val_input_fn(test_dataset_tfrecord,
                                                           batch_size=hparams['batch_size'],
                                                           num_epochs=1),
            steps=hparams['test_steps'], checkpoint_path="./model/model.ckpt-852243")
