...
def get_hyper_parameters(net):
    if net == 'ConvNet':
        config = {'batch_size': batch_size,
                  ...
                  'boundary_epochs': [10, 20, 30], #[epoch for epoch in frange(0, train_epochs, train_epochs/60)][0:60],
                  # factor by which the initial learning rate is multiplied (needs to be one more than the boundaries)
                  'learning_rate_cycle': [0.1, 0.5*0.1, 0.5*0.5*0.1, 0.5*0.5*0.5*0.1], #[learning_rate for learning_rate in frange(10e-5, 1., (1. - 10e-5) / (30. + 2.))][
                  ...
                  'momentum': 1.0,
                  'momentum_cycle': [0.9, 0.9, 0.9, 0.9], #[momentum for momentum in frange(0.95, 0.85, (0.95-0.85)/(30+2))][0:30] + [momentum for momentum in frange(0.85, 0.95, (0.95-0.85)/(30+2))][0:31],
                  'frames': 5,
                  'freq_bins': 229,
                  'num_channels': 1,
                  'num_classes': 88,
